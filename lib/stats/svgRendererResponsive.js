...
function renderStatsSvg(data, options = {}) {
  ...
  const showLanguages = Boolean(options.showLanguages);
  ...
  const topLanguages = Array.isArray(data.topLanguages) ? data.topLanguages.slice(0, 4) : [];
  const hasLanguages = showLanguages && topLanguages.length > 0;
  const shouldShowEmptyState = metrics.length === 0 && !hasLanguages;
  ...
  const emptyStateY = divider1Y + Math.round(54 * layoutScale);
  const languageTitleY = divider2Y + Math.round(34 * layoutScale);
  const languageStartY = languageTitleY + Math.round(28 * layoutScale);
  const languageRowGap = Math.round(28 * layoutScale);

  const avatarBottom = showAvatar ? avatarY + avatarSize : 0;
  const titleBottom = titleY + Math.round(titleFont * 0.9);
  const usernameBottom = userY + Math.round(usernameFont * 0.9);
  const metricsBottom = useVerticalMetrics
    ? metricEndY + Math.round(metricValueFont * 1.2)
    : Math.round(286 * layoutScale) + Math.round(metricValueFont * 1.2);
  const languageBottom = hasLanguages
    ? languageStartY + Math.max(0, topLanguages.length - 1) * languageRowGap + Math.round(14 * layoutScale)
    : 0;
  const footerLabelY = hasLanguages ? languageBottom + Math.round(28 * layoutScale) : labelY;
  const footerProgressY = hasLanguages ? footerLabelY + Math.round(28 * layoutScale) : Math.round(378 * layoutScale);
  const footerBottom = shouldShowEmptyState ? emptyStateY + Math.round(24 * layoutScale) : footerProgressY + 18;
  const contentBottom = Math.max(
    ...
    metricsBottom,
    divider2Y,
    shouldShowEmptyState ? emptyStateY : footerLabelY + Math.round(metricFont * 1.2),
    languageBottom,
    footerBottom
  );
  const autoHeightBuffer = Math.max(32, Math.round(24 * layoutScale));
  const height = hasExplicitHeight ? card.cardHeight : Math.ceil(contentBottom + pad + autoHeightBuffer);
...
<text x="${gx + Math.round(34 * layoutScale)}" y="${Math.round(286 * layoutScale)}" fill="${merged.textColor}" font-size="${metricFont}" font-weight="600">${escapeXml(metric.label)}: <tspan class="numbers" fill="${merged.titleColor}" font-size="${metricValueFont}" font-weight="700">${escapeXml(metric.value)}</tspan></text>
  ...
  const languageNodes = hasLanguages
    ? `<text x="${pad}" y="${languageTitleY}" fill="${merged.titleColor}" font-size="${Math.max(14, Math.round(16 * layoutScale))}" font-weight="700">TOP LANGUAGES</text>
${topLanguages
  .map((lang, index) => {
    const rowY = languageStartY + index * languageRowGap;
    return `<text x="${pad}" y="${rowY}" fill="${merged.textColor}" font-size="${Math.max(13, Math.round(15 * layoutScale))}">${escapeXml(lang.name)}: <tspan class="numbers" fill="${merged.titleColor}" font-weight="700">${escapeXml(lang.count)}</tspan></text>`;
  })
  .join('\n')}`
    : '';

  const emptyStateNode = shouldShowEmptyState
    ? `<text x="${pad}" y="${emptyStateY}" fill="${merged.textColor}" font-size="${Math.max(14, Math.round(18 * layoutScale))}">No metrics selected.</text>`
    : '';

  const footerNode = shouldShowEmptyState
    ? ''
    : `<text x="${pad}" y="${footerLabelY}" fill="${merged.titleColor}" font-size="${Math.max(16, Math.round(18 * layoutScale))}">${footerLabel}: <tspan class="numbers" fill="${merged.titleColor}" font-weight="700">${escapeXml(footerValue)}</tspan></text>

  <rect x="${pad}" y="${footerProgressY}" width="${totalBarW}" height="18" rx="9" fill="rgba(199,215,236,0.26)"/>
  <rect x="${pad}" y="${footerProgressY}" width="${progressW}" height="18" rx="9" fill="url(#bar-${idSuffix})"/>`;
