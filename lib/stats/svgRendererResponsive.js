...
function renderStatsSvg(data, options = {}) {
  ...
  const contentBottom = Math.max(
    ...
    metricsBottom,
    divider2Y,
    shouldShowEmptyState ? emptyStateY : labelY + Math.round(metricFont * 1.2),
    languageBottom,
    0
  );
  const bottomPadding = Math.max(12, Math.round(12 * layoutScale));
  const minCardHeight = 432;
  const height = hasExplicitHeight ? card.cardHeight : Math.max(minCardHeight, Math.ceil(contentBottom + bottomPadding));
  const idSuffix = hashString(`${data.login}|${theme}|${width}|${height}`);

  const footerValue = data.publicRepos;
